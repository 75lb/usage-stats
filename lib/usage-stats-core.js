...
class UsageStats {
  ...
  set dir (val) {
    ...
    try {
      ...
      this._noWritePermission = false
    } catch (err) {
      this._noWritePermission = true
    }
  }
   ...
   */
    ...
    if (this._hits.length === 1) {
      ...
      hit.set('sc', 'end')
      if (sessionParams) hit = joinMaps(hit, sessionParams)
  ...
  _createHit (map, options) {
    ...
    options = options || {}
    let hit = joinMaps(this.defaults, map || new Map())
    if (options.hitParams) hit = joinMaps(hit, options.hitParams)
    if (this._sessionParams) hit = joinMaps(hit, this._sessionParams)
  ...
  _getOSVersion () {
    if (os.platform() === 'win32') {
      ...
      return `(Macintosh; Intel MAC OS X ${os.release()})`
    } else if (os.platform() === 'linux') {
      return `(X11; Linux ${os.release()})`
    }
  }
...
function joinMaps (a, b) {
  return new Map(Array.from(a).concat(Array.from(b)))
}
